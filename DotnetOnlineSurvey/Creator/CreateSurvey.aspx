<%@ Page Title="" Language="C#" MasterPageFile="~/Creator/CreatorMaster.master" AutoEventWireup="true" CodeFile="CreateSurvey.aspx.cs" Inherits="Creator_CreateSurvey" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            $.ajax({
                type: "POST",
                url: "../WebMethods/CreateTakeSurvey.asmx/GetSurveyDetailsById",
                data: "{'surveyId':'" + surveyID + "'}",
                contentType: "application/json",
                dataType: "JSON",
                success: function (response) {
                    //  alert("a");
                    categoryId = response.d.CategoryId;
                },
                failure: function (msg) {
                    alert(msg);
                }
            ...
            $("#QuesType_dropDown").change(function () {
                ...
                if (var1 == 1 || var1 == 2) {
                    ...
                    Answers = 2;
                    $("#Answer_Option").empty();
                    $("#Answer_Option").append('<div><input class="Answer_Option" id="Answer_Option1" style="float:left;width:70%" type="text" placeholder="Enter Your Text" /><br/><br/></div>');
                    $("#Answer_Option").append('<div><input class="Answer_Option" id="Answer_Option2" style="float:left;width:70%" type="text" placeholder="Enter Your Text" /><br/></div>');
                }
                else if (var1 == 4) {
                    $("#Answers_div").show();
                    $("#addanswer").hide();
                    $("#Answer_Option").empty();
                    $("#Answer_Option").append("<input class='Answer_Option' type='text' placeholder='Enter Minimum Value' /><br><br/><input class='Answer_Option' type='text' placeholder='Enter Maximum Value' /><br/><br/><input class='Answer_Option' type='text' placeholder='Enter Interval Value' /><br/>");
                }
                else {
                    Answers = 0;
                    $("#Answers_div").hide();
            ...
            $(document).ready(function () {
                ...
                $("#btnPopupSubmit").click(function () {
                    ...
                    if ($("textarea[id$='question_question']").val() != "") {
                        ...
                        if (($('#Answer_Option1').val() == '') || ($('#Answer_Option2').val() == '')) {
                            alert('Please enter answer Value');

                        }
                        else {

                            $('#sortable').append('<div data-qType="' + typeDiv + '" class="qBox" id="questionDiv' + i + '"><br/><input class="required" type="checkbox" style="float:right" value="' + i + '" name="required" /><b><p align="right" style="color:red">Mark As Required&nbsp;&nbsp;</p></b><br/><img class="btnEdit" src="~/images/edit.png" style="float:right;right:0px;height:50px;width:50px" /><img src="~/images/delete.png" class="btnDelete" style="float:right;right:0px;height:50px;width:50px" /><span class="qText">' + $("textarea[id$='question_question']").val() + '</span>' + opt + '</div>');
                            i++;
                ...
                $("#addanswer").click(function () {
                    if (Answers < 8) {
                        Answers++;
                        //        alert(Answers);
                        $("#Answer_Option").append('<div><br/><input class="Answer_Option" id="Answer_Option' + Answers + '" style="float:left;width:70%" type="text" placeholder="Enter Your Text" /><input onclick="javascript:CloseClick(this);" style="float:right;height:20px;vertical-align:top" type="button" value="X" /></div><br/>');
                    }
                    else
                ...
                $("#saveSurveyToDb").click(function () {
                    var newName = $("#<%=Survey_Title.ClientID%>").text();
                    var questionSrno = 0;
                    ...
                    $(".qBox").each(function () {
                        ...
                        if (true) {
                            $.ajax({
                                ...
                                data: "{'categoryID':'" + categoryId + "','creatorID':'" + creatorID + "','surveyID':'" + surveyID + "','questionTypeId':'" + questionTypeId + "','questionSrNo':'" + questionSrno + "','question':'" + question + "','required':'" + required + "','minVal':'" + minVal + "','maxVal':'" + maxVal + "','intrval':'" + intrval + "','options':'" + options + "'}",
                                contentType: "application/json",
                                dataType: "JSON",
                                success: function (response) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../WebMethods/CreateTakeSurvey.asmx/UpdateSurveyName",
                        data: "{'survey':'" + surveyID + "', 'title':'" + newName + "'}",
                        contentType: "application/json",
                        dataType: "JSON",
                        success: function (response) {
                            window.location = "PreviewSurvey.aspx?surveyID=" + surveyID;
                        },
                        failure: function (msg) {
                            alert(msg);
                        }
    ...
    <h1>
        <asp:Label ID="Survey_Title" runat="server">Nacre Survey</asp:Label>
    ...
    <a href="#" id="saveSurveyToDb" class="myButton">Save Survey</a>
    ...
    <div id="popupcreateques" style="display: none">
        <span class="button b-close"><span>X</span></span>
            ...
            <div   id="Answers_div" style="display:none">
               <br /><div align="center"> <input type="button" id="addanswer" value="Add More Options" title="Add Answer" /></div>
               <br /><div id="Answer_Option">
            ...
            --%></div></div><br /><div align ="center"><input id="btnPopupSubmit" type="button" style="width:80px;height:30px;color:red;border-bottom-style:double;vertical-align:top" value="Save" onclick="if (!ValidateQuestion()) { return false; }" /></div>
